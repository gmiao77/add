...
export const strategy3 = {
    ...
    getHtml() {
        return `
            <header class="mb-6">
                <h2 class="text-xl font-bold text-gray-800">亚马逊自动广告批量创建模板1.3</h2>
            </header>
                ...
                if (inputs.sku_independent) {
                    ...
                    bids.forEach(bid => {
                        matchingTypes.forEach(m_type => {
                            // 构建活动和广告组名称
                            const campaignNameWithSuffix = `${inputs["广告活动名称"]}_${bid}_${m_type}_${suffix}`;
                            const adGroupNameWithSuffix = `${inputs["广告组名称"]}_${bid}_${m_type}_${suffix}`;
                            ...
                            // 根据匹配类型生成产品定位行
                            if (m_type === "0") {
                                const targetingTypes = ["close-match", "substitutes", "loose-match", "complements"];
                                targetingTypes.forEach(targetingType => {
                                    const rowStatus = ["close-match", "substitutes"].includes(targetingType) ? "enabled" : "paused";
                                    rows.push([
                                        "Sponsored Products", "Product Targeting", "Create", campaignNameWithSuffix, adGroupNameWithSuffix, "", "", "", "", 
                                        "", "", "", "", "", rowStatus, "", 
                                        "", "", bid, "", "", "", "", "", "", "", targetingType
                                    ]);
                                });
