// 策略7模块 - Amazon ASIN定投广告批量创建-群友自定义测试版
export const strategy7 = {
    // 存储ASIN数据
    asins: [],
    ...
    init(container) {
        ...
        // 显示初始化状态
        this.showStatus('策略7已加载，可开始配置', 'success');
    },
    ...
    getHtml() {
        return `
            <header class="mb-6">
                <h2 class="text-xl font-bold text-gray-800">ASIN定投广告批量创建-群友自定义测试版</h2>
            </header>
            ...
            <form id="adForm" class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="space-y-2">
                            <label class="inline-flex items-center">
                                <input type="checkbox" name="投放类型" value="精准投放" checked
                                    class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                <span class="ml-2 text-sm text-gray-700">精准投放</span>
                            </label>
                            <label class="inline-flex items-center">
                                <input type="checkbox" name="投放类型" value="拓展投放" checked
                                    class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                <span class="ml-2 text-sm text-gray-700">拓展投放</span>
                            </label>
                        </div>
                    </div>
                    ...
                    <div class="space-y-2">
                        <label class="block text-sm font-medium text-gray-700">复制对象</label>
                        <div class="space-y-2">
                            <label class="inline-flex items-center">
                                <input type="radio" name="复制对象" value="广告活动" checked
                                    class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                <span class="ml-2 text-sm text-gray-700">广告活动</span>
                            </label>
                            <label class="inline-flex items-center">
                                <input type="radio" name="复制对象" value="广告组"
                                    class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                <span class="ml-2 text-sm text-gray-700">广告组</span>
                            </label>
                        </div>
                    </div>
                </div>
    ...
    generateTemplate() {
        try {
            ...
            // 验证投放类型至少选择一个
            if (!inputs['投放类型'] || inputs['投放类型'].length === 0) {
                throw new Error("请至少选择一种投放类型");
            }
                    ...
                    if (inputs.sku_independent) {
                        ...
                        bids.forEach(bid => {
                            groupedAsins.forEach((group, gIdx) => {
                                ...
                                // ASIN目标行（处理所有选中的投放类型）
                                inputs['投放类型'].forEach(targetType => {
                                    group.forEach(asin => {
                                        const targetingExpr = targetType === "精准投放" 
                                            ? `asin="${asin}"` 
                                            : `asin-expanded="${asin}"`;
                ...
                if (inputs.sku_independent) {
                    ...
                    bids.forEach(bid => {
                        groupedAsins.forEach((group, gIdx) => {
                            ...
                            // 广告活动行（只创建一次）
                            const campaignId = `${inputs['广告活动名称']}_${gSuffix}_${bid}`;
                            rows.push([
                                "Sponsored Products", "Campaign", "Create", campaignId, "", "", "", "", "",
                                campaignId, "", today, "", "MANUAL", "enabled", inputs["预算"], 
                                "", "", "", "", "", "", "", inputs["竞价策略"], "", "", ""
                    ...
                    skus.forEach(sku => {
                        bids.forEach(bid => {
                            groupedAsins.forEach((group, gIdx) => {
                                ...
                                // 广告活动行（只创建一次）
                                const campaignId = `${inputs['广告活动名称']}_${sku}_${gSuffix}_${bid}`;
                                rows.push([
                                    "Sponsored Products", "Campaign", "Create", campaignId, "", "", "", "", "",
                                    campaignId, "", today, "", "MANUAL", "enabled", inputs["预算"], 
                                    "", "", "", "", "", "", "", inputs["竞价策略"], "", "", ""
